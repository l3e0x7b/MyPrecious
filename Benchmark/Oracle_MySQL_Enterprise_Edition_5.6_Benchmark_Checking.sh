#!/bin/bash
##
# MySQL 基线检测脚本，基于 CIS_Oracle_MySQL_Enterprise_Edition_5.6_Benchmark_v1.1.0
...
echo -n "3.9 检查审计日志文件是否有适当的权限" | tee -a ${REPORT}
audit_stat=`${MY_EXEC} -e "select plugin_status from information_schema.plugins where plugin_name='audit_log';" | awk '{if (NR!=1) print}'`
if [[ ${audit_stat} = "ACTIVE" ]]; then
	auditlogid=0
	auditlog=`${MY_EXEC} -e "show variables like 'audit_log_file';" | awk '{if (NR!=1) print $2}'`
	if [[ ${auditlog} = "" ]]; then
		echo -e "\n\t未配置审计日志，跳过检查。" >> ${REPORT}
	else
		perm=`stat -c %a/%U/%G ${auditlog}`
		if [[ ${perm} != "660/mysql/mysql" ]]; then
			echo -e "\n\t审计日志 (${auditlog}) 的权限未按建议配置，当前权限如下：" >> ${REPORT}
			stat ${auditlog} | grep Access | head -n1 | sed 's/^/\t/g' >> ${REPORT}
		else
			echo -e "\n\t审计日志 (${auditlog}) 有适当的权限。" >> ${REPORT}
		fi
	fi
else
	auditlogid=1
	echo -e "\n\t审计插件 audit_log 未启用，跳过检查。" >> ${REPORT}
fi
echo "..........[OK]"
echo "" >> ${REPORT}
...
echo -n "6.4 检查 log-raw 是否已设置为 OFF" | tee -a ${REPORT}
...
echo -n "6.5 检查 audit_log_connection_policy 是否未设置为 NONE" | tee -a ${REPORT}
if [[ ${auditlogid} -eq 0 ]]; then
	conn_policy=`${MY_EXEC} -e "show variables like 'audit_log_connection_policy';" | awk '{if (NR!=1) print $2}'`
	if [[ ${conn_policy} = "NONE" ]]; then
		echo -e "\n\taudit_log_connection_policy 已设置为 NONE。" >> ${REPORT}
	else
		echo -e "\n\taudit_log_connection_policy 未设置为 NONE。" >> ${REPORT}
	fi
else
	echo -e "\n\t审计插件 audit_log 未启用，跳过检查。" >> ${REPORT}
fi
...
echo -n "6.6 检查 audit_log_exclude_accounts 是否未设置为 NULL" | tee -a ${REPORT}
if [[ ${auditlogid} -eq 0 ]]; then
	exc_accounts=`${MY_EXEC} -e "show variables like 'audit_log_exclude_accounts';" | awk '{if (NR!=1) print $2}'`
	if [[ ${exc_accounts} = "NULL" ]]; then
		echo -e "\n\taudit_log_exclude_accounts 已设置为 NULL。" >> ${REPORT}
	else
		echo -e "\n\taudit_log_exclude_accounts 未设置为 NULL。" >> ${REPORT}
	fi
else
	echo -e "\n\t审计插件 audit_log 未启用，跳过检查。" >> ${REPORT}
fi
...
echo -n "6.7 检查 audit_log_include_accounts 是否已设置为 NULL" | tee -a ${REPORT}
if [[ ${auditlogid} -eq 0 ]]; then
	inc_accounts=`${MY_EXEC} -e "show variables like 'audit_log_include_accounts';" | awk '{if (NR!=1) print $2}'`
	if [[ ${inc_accounts} = "NULL" ]]; then
		echo -e "\n\taudit_log_include_accounts 已设置为 NULL。" >> ${REPORT}
	else
		echo -e "\n\taudit_log_include_accounts 未设置为 NULL。" >> ${REPORT}
	fi
else
	echo -e "\n\t审计插件 audit_log 未启用，跳过检查。" >> ${REPORT}
fi
...
echo -n "6.8 检查 audit_log_policy 是否已设置为记录登录" | tee -a ${REPORT}
if [[ ${auditlogid} -eq 0 ]]; then
	policy_login=`${MY_EXEC} -e "show variables like 'audit_log_policy';" | awk '{if (NR!=1) print $2}'`
	if [[ ${policy_login} =~ LOGINS|ALL ]]; then
		echo -e "\n\taudit_log_policy 已设置为记录登录。" >> ${REPORT}
	else
		echo -e "\n\taudit_log_policy 未设置为记录登录。" >> ${REPORT}
	fi
else
	echo -e "\n\t审计插件 audit_log 未启用，跳过检查。" >> ${REPORT}
fi
...
echo -n "6.9 检查 audit_log_policy 是否已设置为记录登录和连接" | tee -a ${REPORT}
if [[ ${auditlogid} -eq 0 ]]; then
	policy_all=`${MY_EXEC} -e "show variables like 'audit_log_policy';" | awk '{if (NR!=1) print $2}'`
	if [[ ${policy_all} = "ALL" ]]; then
		echo -e "\n\taudit_log_policy 已设置为记录登录和连接。" >> ${REPORT}
	else
		echo -e "\n\taudit_log_policy 未设置为记录登录和连接。" >> ${REPORT}
	fi
else
	echo -e "\n\t审计插件 audit_log 未启用，跳过检查。" >> ${REPORT}
fi
...
echo -n "6.10 检查 audit_log_statement_policy 是否已设置为 ALL" | tee -a ${REPORT}
if [[ ${auditlogid} -eq 0 ]]; then
	statement_policy=`${MY_EXEC} -e "show variables like 'audit_log_statement_policy';" | awk '{if (NR!=1) print $2}'`
	if [[ ${statement_policy} = "ALL" ]]; then
		echo -e "\n\taudit_log_statement_policy 已设置为 ALL。" >> ${REPORT}
	else
		echo -e "\n\taudit_log_statement_policy 未设置为 ALL。" >> ${REPORT}
	fi
else
	echo -e "\n\t审计插件 audit_log 未启用，跳过检查。" >> ${REPORT}
fi
...
echo -n "6.11 检查 audit_log_strategy 是否已设置为 SYNCHRONOUS 或 SEMISYNCRONOUS" | tee -a ${REPORT}
if [[ ${auditlogid} -eq 0 ]]; then
	strategy=`${MY_EXEC} -e "show variables like 'audit_log_strategy';" | awk '{if (NR!=1) print $2}'`
	if [[ ${strategy} =~ SYNCHRONOUS|SEMISYNCRONOUS ]]; then
		echo -e "\n\taudit_log_strategy 已设置为 SYNCHRONOUS 或 SEMISYNCRONOUS。" >> ${REPORT}
	else
		echo -e "\n\taudit_log_strategy 未设置为 SYNCHRONOUS 或 SEMISYNCRONOUS。" >> ${REPORT}
	fi
else
	echo -e "\n\t审计插件 audit_log 未启用，跳过检查。" >> ${REPORT}
fi
...
echo -n "6.12 检查审计插件是否无法被卸载" | tee -a ${REPORT}
if [[ ${auditlogid} -eq 0 ]]; then
	load_opt=`${MY_EXEC} -e "select load_option from information_schema.plugins where plugin_name='audit_log';" | awk '{if (NR!=1) print}'`
	if [[ ${load_opt} = "FORCE_PLUS_PERMANENT" ]]; then
		echo -e "\n\t审计插件是否无法被卸载。" >> ${REPORT}
	else
		echo -e "\n\t审计插件是否可以被卸载。" >> ${REPORT}
	fi
